...
from ThresholdModel_py3 import *
from Filter_Rect_LogSpaced_py3 import *

from Tools_py3 import reprint
import cython_helpers as cyth


class GIF_py3(ThresholdModel_py3) :
    ...
    def __init__(self, dt=0.1):
        ...
        self.eta     = Filter_Rect_LogSpaced_py3()    # nA, spike-triggered current (must be instance of class Filter)
        self.gamma   = Filter_Rect_LogSpaced_py3()    # mV, spike-triggered movement of the firing threshold (must be instance of class Filter)
    ...
    def simulate(self, I, V0):
        ...
        for t in np.arange(p_T-1):
            ...
            ## COMPUTE PROBABILITY OF EMITTING ACTION POTENTIAL
            temp_lambda = p_lambda0 * np.exp( (V[t+1] - p_Vt_star - gamma_sum[t]) / p_DV )
            p_dontspike = np.exp(-temp_lambda*(p_dt/1000.0))                                  ## since lambda0 is in Hz, dt must also be in Hz (this is why dt/1000.0)
    ...
    def simulate_cython(self, I, V0):
        ...
        V, eta_sum, gamma_sum, spks = cyth.c_simulate(p_T, p_dt, p_gl, p_C, p_El, p_Vr, p_Tref, p_Vt_star,
                                                      p_DV, p_lambda0, V, I, p_eta, p_eta_l, eta_sum, p_gamma, gamma_sum, p_gamma_l, spks)
    ...
    def simulateDeterministic_forceSpikes(self, I, V0, spks):
        ...
        spks_i   = Tools_py3.timeToIndex(spks, self.dt)
        ...
        v = weave.inline(code, vars)
        '''
        next_spike = spks_i[0] + int(p_Tref/p_dt)
        spks_cnt = 0
        ...
        for t in np.arange(p_T-1):
            ...
            if ( t == next_spike ):
                spks_cnt = spks_cnt + 1
                if not spks_cnt > (np.size(spks_i)-1):
                    next_spike = spks_i[spks_cnt] + int(p_Tref/p_dt)
                V[t-1] = 0
                V[t] = p_Vr
                t= t-1
    ...
    def simulateDeterministic_forceSpikes_cython(self, I, V0, spks):
    ...
    def fit_cython(self, experiment, DT_beforeSpike = 5.0):
        ...
        self.fitSubthresholdDynamics_cython(experiment, DT_beforeSpike=DT_beforeSpike)
        
        self.fitStaticThreshold_cython(experiment)

        self.fitThresholdDynamics_cython(experiment)
    ...
    def fitSubthresholdDynamics_cython(self, experiment, DT_beforeSpike=5.0):
        ...
        for tr in experiment.trainingset_traces :
            ...
            if tr.useTrace :
                ...
                # Simulate subthreshold dynamics 
                (time, V_est, eta_sum_est) = self.simulateDeterministic_forceSpikes_cython(tr.I, tr.V[0], tr.getSpikeTimes())
    ...
    def fitStaticThreshold_cython(self, experiment):
        ...
        # Perform fit     
        beta0_staticThreshold = [1/self.DV, -self.Vt_star/self.DV] 
        beta_opt = self.maximizeLikelihood_cython(experiment, beta0_staticThreshold, self.buildXmatrix_staticThreshold) 
    ...
    def fitThresholdDynamics_cython(self, experiment):
        ...
        # Perform fit        
        beta0_dynamicThreshold = np.concatenate( ( [1/self.DV], [-self.Vt_star/self.DV], self.gamma.getCoefficients()/self.DV))        
        beta_opt = self.maximizeLikelihood_cython(experiment, beta0_dynamicThreshold, self.buildXmatrix_dynamicThreshold)
    ...
    def maximizeLikelihood_cython(self, experiment, beta0, buildXmatrix, maxIter=10**3, stopCond=10**-6) :
    ...
    def plotAverageModel(cls, GIFs):
        ...
        Tools_py3.removeAxis(plt.gca(), ['top', 'right'])
        plt.xlim([K_support[0], K_support[-1]])
        plt.xlabel('Time (ms)')
        plt.ylabel('Membrane filter (MOhm/ms)')  
        ...
        plt.xlabel('R (MOhm)')
        Tools_py3.removeAxis(plt.gca(), ['top', 'left', 'right'])
        plt.yticks([])        
