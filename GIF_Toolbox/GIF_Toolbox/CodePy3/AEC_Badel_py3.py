...
from Filter_Rect_LinSpaced_py3 import *
from Filter_Rect_LogSpaced_AEC_py3 import *
...
class AEC_Badel_py3(AEC_py3) :
        
        
    def __init__(self, dt):
        
        # Define variables for optimal linear filter K_opt   
        self.K_opt      = Filter_Rect_LogSpaced_AEC_py3(length=150.0, binsize_lb=dt, binsize_ub=5.0, slope=10.0, clamp_period=0.5)
        self.K_opt_all  = []                # List of K_opt, store bootstrap repetitions                         

        # Define variables for electrode filter    
        self.K_e        = Filter_Rect_LinSpaced_py3()
    ...
    def computeElectrodeFilter(self, expr) :
        ...
        for rep in np.arange(self.p_nbRep) :
            ...
            Ke_tmp = Filter_Rect_LinSpaced_py3(length=self.p_Ke_l, nbBins=len(Ke_coeff_tmp))
        ...
        self.K_opt = Filter_py3.averageFilters(self.K_opt_all)
        self.K_e = Filter_py3.averageFilters(self.K_e_all)   
    ...
    def plot(self):
        ...
        Filter_py3.plotAverageFilter(self.K_opt_all, 0.05, loglog=False, label_x='Time (ms)', label_y='Optimal linear filter (MOhm/ms)')
 
        # Plot optimal linear filter K_e        
        Filter_py3.plotAverageFilter(self.K_e_all, 0.05, label_x='Time (ms)', label_y='Electrode filter (MOhm/ms)')
    ...
    def plotKe(self):
        ...
        # Plot optimal linear filter K_e        
        Filter_py3.plotAverageFilter(self.K_e_all, 0.05, label_x='Time (ms)', label_y='Electrode filter (MOhm/ms)', plot_expfit=False)
