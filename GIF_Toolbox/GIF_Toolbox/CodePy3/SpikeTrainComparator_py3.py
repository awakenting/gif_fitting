...
class SpikeTrainComparator_py3 :
    ...
    def computeMD_Kistler(self, delta, dt) :        
        ...
        KistlerDotProduct = SpikeTrainComparator_py3.Md_dotProduct_Kistler
    ...
    def computeMD_Rect(self, delta, dt) :        
        ...
        RectDotProduct = SpikeTrainComparator_py3.Md_dotProduct_Rect
    ...
    def computeMD(self, dotProduct, dotProductArgs, dt) :
        ...
        for s in self.spks_data :
            ...
            spike_train_tmp = SpikeTrainComparator_py3.getSpikeTrain(s, T, dt)
            all_spike_train_data.append(spike_train_tmp)
    
        # Compute average spike-strain for both sets
        spiketrain_data_avg = SpikeTrainComparator_py3.getAverageSpikeTrain(self.spks_data, T, dt) 
        spiketrain_model_avg = SpikeTrainComparator_py3.getAverageSpikeTrain(self.spks_model, T, dt)        
    ...
    def plotRaster(self, delta=10.0, dt=0.1):
        ...
        spks_avg_data         = SpikeTrainComparator_py3.getAverageSpikeTrain(self.spks_data, self.T, dt)
        spks_avg_data_support = np.arange(len(spks_avg_data))*dt
        spks_avg_data_smooth  = fftconvolve(spks_avg_data, rect_window, mode='same')
           
        spks_avg_model = SpikeTrainComparator_py3.getAverageSpikeTrain(self.spks_model, self.T, dt)
