...
class Trace_auto_read_T :
    ...
    def __init__(self, V, V_units, I, I_units, FILETYPE='Igor', T=1000, dt=0.1, V_d = 0, V_d_units = 1e-3, I_d = 0, I_d_units = 1e-9):
        ...
        if FILETYPE=='Igor' :
            ...
            V_rec, ibw_T, ibw_dt       = ReadIBW_return_length_and_dt.read(V)
            self.dt = ibw_dt
            self.T = ibw_T
            self.V_rec  = np.array(V_rec[:int(ibw_T/self.dt)])*V_units/10**-3        # convert voltage trace to mV
                        
            I, i_ibw_T, i_ibw_dt           = ReadIBW_return_length_and_dt.read(I)
            self.I     = np.array(I[:int(i_ibw_T/i_ibw_dt)])*I_units/10**-9             # convert input current trace to nA
            
            if not type(V_d) == int:
                V_d_rec, ibw_T, ibw_dt       = ReadIBW_return_length_and_dt.read(V_d)
                self.V_d_rec  = np.array(V_d_rec[:int(ibw_T/self.dt)])*V_d_units/10**-3        # convert voltage trace to mV
                            
                I_d, ibw_T, ibw_dt           = ReadIBW_return_length_and_dt.read(I_d)
                self.I     = np.array(I_d[:int(ibw_T/self.dt)])*I_d_units/10**-9             # convert input current trace to nA
