...
class GIF_subadapt_constrained(ThresholdModel) :
    ...
    def __init__(self, dt=0.1):
        ...
        self.tau_w   = 10               # time constant of subthreshold adaptation current W
    ...
    def simulate(self, I, V0):
        ...
        V, eta_sum, gamma_sum, spks = cyth.c_simulate(p_T, p_dt, p_gl, p_C, p_El, p_Vr, p_Tref,\
                                                      p_Vt_star, p_DV, p_lambda0, V, I, p_eta,\
                                                      p_eta_l, eta_sum, p_gamma, gamma_sum,\
                                                      p_gamma_l, spks)
...
#        self.a_w = -b[-1]*self.C
        ...
        def func(p, pX=X, pY=Y, ptau_w = 10):
            error = np.mean((pY - np.dot(pX,p))**2)
            return error
        
        
        # constraints are:
        # 1. C/g_l = tau_m < tau_w  <=>   tau_w - (C/g_l) >= 0
        # 2. C/g_l >= 0
        ...
        res = minimize(func, p0, args=(X, Y, tau_w), constraints=cons, method='SLSQP', 
                       options={'disp': True, 'maxiter': 1000}, callback=get_params, bounds = bonds)
        ...
        self.C  = 1./p[1]
        self.gl = -p[0]*self.C
        self.El = p[2]*self.C/self.gl
        self.eta.setFilter_Coefficients(-p[3:-1]*self.C)
        self.a_w = -p[-1]*self.C
        ...
        var_explained_dV = 1.0 - np.mean((Y - np.dot(X,p))**2)/np.var(Y)
...
#        Z_py = np.array(np.zeros(p_T), dtype="double")
#        
#        # initial value        
#        Z_py[0] = 0
#        t = 0
#        while(t < (p_T-1)):
#            Z_py[t+1] = Z_py[t] + p_dt/p_tau_w * (V[t] - p_Ew - Z_py[t])
#            t += 1
            
#        plt.figure()
#        plt.plot(Z, hold=True)
#        plt.plot(trace.V)
#        plt.legend(['W','V'])
#        plt.title('Tau: ' + str(self.tau_w))
    ...
    def fitStaticThreshold(self, experiment):
        ...
        print ("\nGIF_subadapt MODEL - Fit static threshold...\n")
    ...
    def fitThresholdDynamics(self, experiment):
        ...
        print ("\nGIF_subadapt MODEL - Fit dynamic threshold...\n")
    ...
    def computeLikelihoodGradientHessian(self, beta, X, X_spikes, sum_X_spikes) : 
        ...
        L = np.sum(X_spikesbeta) - self.lambda0*dt*np.sum(expXbeta)
    ...
    def printParameters(self):
        ...
        print ("tau_w (ms):\t%0.3f"  % (self.tau_w))
    ...
    @classmethod
    def compareModels(cls, GIF_subadapts, labels=None):

        """
        Given a list of GIF_subadapt models, GIF_subadapts, the function produce a plot in which the model parameters are compared.
        """

        # PRINT PARAMETERS        

        print ("\n#####################################")
        print ("GIF_subadapt model comparison")
        print ("#####################################\n")
        
        cnt = 0
        for GIF_subadapt in GIF_subadapts :
            
            print ("Model: " + labels[cnt])          
            GIF_subadapt.printParameters()
            cnt+=1
        ...
        colors = plt.cm.jet( np.linspace(0.7, 1.0, len(GIF_subadapts) ) )   
        ...
        for GIF_subadapt in GIF_subadapts :
            ...
            K = 1./GIF_subadapt.C*np.exp(-K_support/(GIF_subadapt.C/GIF_subadapt.gl))     
        ...
        for GIF_subadapt in GIF_subadapts :
            ...
            (eta_support, eta) = GIF_subadapt.eta.getInterpolatedFilter(0.1)         
        ...
        for GIF_subadapt in GIF_subadapts :
            ...
            V_support = np.linspace(GIF_subadapt.Vt_star-5*GIF_subadapt.DV,GIF_subadapt.Vt_star+10*GIF_subadapt.DV, 1000) 
            escape_rate = GIF_subadapt.lambda0*np.exp((V_support-GIF_subadapt.Vt_star)/GIF_subadapt.DV)                
        ...
        for GIF_subadapt in GIF_subadapts :
            ...
            (gamma_support, gamma) = GIF_subadapt.gamma.getInterpolatedFilter(0.1)         
    ...
    @classmethod
    def plotAverageModel(cls, GIF_subadapts):
        ...
        for GIF_subadapt in GIF_subadapts :
            ...
            (K_support, K) = GIF_subadapt.eta.getInterpolatedFilter(0.1)      
        ...
        for GIF_subadapt in GIF_subadapts :
            ...
            (K_support, K) = GIF_subadapt.gamma.getInterpolatedFilter(0.1)      
        ...
        for GIF_subadapt in GIF_subadapts :
            ...
            p = GIF_subadapt.C/GIF_subadapt.gl
            p_all.append(p)
